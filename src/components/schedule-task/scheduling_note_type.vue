...
// create_user	create_user	int			FALSE	FALSE	FALSE -->
    ...
    <el-dialog width="500px" :title="addSntText" :close-on-click-modal="false" :visible.sync="addsntVisiable"
      ...
      <zj-form :model="sntModel" :newDataFlag='addsntVisiable' label-width="100px" ref="sntForm"  :rules="add_rules">
        <!-- <el-form-item label="序号">
          <el-input class="formItem" v-model="sntModel.snt_id" placeholder="系统自动生成" disabled>
          </el-input>
        </el-form-item> -->
        <el-form-item label="标记名称" prop="snt_name">
          <el-input class="formItem" v-model="sntModel.snt_name" placeholder="请填写标记类型名称">
          </el-input>
        </el-form-item>
        <el-form-item label="标记类型说明" prop="snt_note">
...
onDeleteClick(row) {
      this.$confirm("是否删除？", "提示", {
        ...
        .then(() => {
          this.z_delete("api/scheduling_note_type", { data: row })
            .then(res => {
              this.$message({
                message: "删除成功",
                type: "success",
